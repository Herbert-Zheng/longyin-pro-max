...
Require-Pattern $source 'var\s+changeFavorPatched\s*=\s*PatchMethod\([^;]+nameof\(ChangeFavorPrefix\),\s*nameof\(ChangeFavorPostfix\)\)' 'Relationship bonus must patch both the value-scaling prefix and the post-success notification postfix.'
...
Require-Pattern $updatePostfix 'try\s*\{[\s\S]*?ApplyViewedHeroFavorTest\(\);[\s\S]*?\}\s*finally\s*\{\s*FlushDeferredRelationshipBonusLogs\(\);\s*\}' 'The main game update postfix must flush deferred relationship notifications from a finally block after its normal update work.'
