...
Reject-Pattern $showPostfix 'Math\.Abs\(\s*_yellowCraneFinishRecruitLevel\s*-\s*__2\s*\)[\s\S]{0,40}<' 'A real Yellow Crane open can report FinishRecruitHero Normal-3 and same-frame ShowRecruitUI Normal/4/3.6; session activation must not require the parsed finish level to equal the actual ShowRecruitUI level.'
