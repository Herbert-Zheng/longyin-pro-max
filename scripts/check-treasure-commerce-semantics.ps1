...
Require-ScopePattern $tradeAnalyzerMethod 'buyPrice\s*=\s*Math\.Max\(0,\s*TryGetTradePriceForItem\(icon,\s*item,\s*buy:\s*true,\s*fallback:\s*0\)\);[\s\S]*?if\s*\(buyPrice\s*<=\s*0\)\s*\{\s*return false;' 'Automatic shopping must reject an item when its authoritative native buy price is unavailable instead of treating the fallback zero as free.'
Reject-ScopePattern $tradeAnalyzerMethod 'buyPrice\s*<=\s*0\s*&&\s*currentSellPrice\s*<=\s*0' 'A failed buy-price lookup must not be masked by a successful informational sell-price lookup.'
...
Require-ScopePattern $appraisedValueMethod 'appraisedValue\s*=\s*Math\.Max\(0,\s*item\.GetTreasureValue\(guess:\s*true\)\);\s*return true;' 'The shared appraisal helper must read the same guessed value that QuickDetail renders inside parentheses.'
...
Reject-ScopePattern $appraisedValueMethod 'item\.GetTreasureRealValue\s*\(' 'The hidden real treasure value must never be used as the player-visible parenthesized appraisal.'
