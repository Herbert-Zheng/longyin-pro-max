...
Require-Pattern $updatePostfix 'skillDetailVisible\s*=\s*__instance\.skillDetail\s*!=\s*null\s*&&\s*__instance\.skillDetail\.activeInHierarchy[\s\S]*?bookDetailVisible\s*=\s*__instance\.bookDetail\s*!=\s*null\s*&&\s*__instance\.bookDetail\.activeInHierarchy[\s\S]*?if\s*\(\s*!skillDetailVisible\s*&&\s*!bookDetailVisible\s*\)[\s\S]*?ResetSkillBookOwnershipAppliedLabel\(\)[\s\S]*?return[\s\S]*?ResolveVisibleSkillDetailId[\s\S]*?_skillBookOwnershipAppliedSkillId\s*==\s*skillId[\s\S]*?IndexOf\("功法书："[\s\S]*?TryApplySkillBookOwnershipToVisibleDescription' 'The update hook must accept either active learned-skill or backpack-book detail, reject when both are hidden, remain idempotent, and apply the resolved skill ID.'
